package com.lynch.binary_indexed_tree;

import org.junit.Test;

/**
 * https://github.com/algorithmzuo/algorithmbasic2020/blob/master/src/class32/Code02_IndexTree2D.java
 * @author: lynch
 * @description:
 * @date: 2024/2/26 23:47
 */
public class IndexTree2D {
//    public IndexTree2D(){}

    @Test
    public void test(){
        int[][] matrix = {
            {3, 0, 1, 4, 2},
            {5, 6, 3, 2, 1},
            {1, 2, 0, 1, 5},
            {4, 1, 0, 1, 7},
            {1, 0, 3, 0, 5},
        };

        IndexTree2DInternal indexTree2D = new IndexTree2DInternal(matrix);
        int result = indexTree2D.sumRegion(2, 1, 4, 3);
        System.out.println(result); // 8

        indexTree2D.update(3, 2, 2);

        result = indexTree2D.sumRegion(2, 1, 4, 3);
        System.out.println(result); // 10
    }

    static class IndexTree2DInternal {
        private int[][] tree;
        private int[][] nums;
        private int N;
        private int M;

        public IndexTree2DInternal(int[][] matrix) {
            if (matrix.length == 0 || matrix[0].length == 0) {
                return;
            }
            N = matrix.length;
            M = matrix[0].length;
            tree = new int[N + 1][M + 1];
            nums = new int[N][M];
            for (int i = 0; i < N; i++) {
                for (int j = 0; j < M; j++) {
                    update(i, j, matrix[i][j]);
                }
            }
        }

        private int sum(int row, int col) {
            int sum = 0;
            for (int i = row + 1; i > 0; i -= i & (-i)) {
                for (int j = col + 1; j > 0; j -= j & (-j)) {
                    sum += tree[i][j];
                }
            }
            return sum;
        }

        public void update(int row, int col, int val) {
            if (N == 0 || M == 0) {
                return;
            }
            int add = val - nums[row][col];
            nums[row][col] = val;
            for (int i = row + 1; i <= N; i += i & (-i)) {
                for (int j = col + 1; j <= M; j += j & (-j)) {
                    tree[i][j] += add;
                }
            }
        }

        public int sumRegion(int row1, int col1, int row2, int col2) {
            if (N == 0 || M == 0) {
                return 0;
            }
            return sum(row2, col2) + sum(row1 - 1, col1 - 1) - sum(row1 - 1, col2) - sum(row2, col1 - 1);
        }
    }
}
