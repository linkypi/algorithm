        ...
        for (int i = 1; i <= n; i++) {
            for (int j = 1; j <= m; j++) {
                if (str1.charAt(i-1) == str2.charAt(j-1)) {
                    ...
                    // 1. str1[i] 不在公共子序列，str2[j] 在公共子序列，此时的公共长度只能从str1 i-1 位取到 即 dp[i-1][j]
                    // 2. str1[i] 在公共子序列，str2[j] 不在公共子序列，此时的公共长度只能从str2 j-1 位取到 即 dp[i][j-1]
                    // 3. str1[i]及str2[j]都不在公共子序列，str1，str2的公共长度取决于两者的前一个位置的公共长度 即 dp[i-1][j-1]
                    //    由于该情况的公共长度必定小于前两种情况，故可以忽略该情况
                    // 综合以上情况
