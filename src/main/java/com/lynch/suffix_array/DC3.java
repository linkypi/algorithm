...
// 也需要 O(N*lgN) 的复杂度，所以其复杂度实在太高，难以接受。为了可以更快求出所有后缀子串的字典序排名，所以有了DC3算法
// (复杂度O(N))其使用了类似分治的算法思想将本应复杂的计算过程巧妙的拆分为小单元计算，最后再合并得到最终结果。
//
...
// 而他们为了决策出最终排名，此时就需要索引1子串后面的信息，其后面的信息刚好就是索引4的子串。同样，索引4子串后面的信息
